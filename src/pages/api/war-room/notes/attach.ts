import type { NextApiRequest, NextApiResponse } from "next";
import { getRedis } from "@/lib/warroom/redis";
import { auditFromEvent } from "@/lib/warroom/audit";

export const dynamic = "force-dynamic";

const EVENT_KEY = (id: string) => `kiq:warroom:event:${id}`;

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== "POST") {
    return res.status(405).json({ ok: false, error: "Method not allowed" });
  }

  const redis = getRedis();
  const { eventId, attachment } = req.body;

  if (!eventId || !attachment) {
    return res.status(400).json({ ok: false, error: "eventId and attachment required" });
  }

  const rawEvent = await redis.get(EVENT_KEY(eventId));
  if (!rawEvent) {
    return res.status(404).json({ ok: false, error: "Event not found" });
  }

  const event = JSON.parse(rawEvent);
  
  const newAttachment = {
    id: `att-${Date.now()}`,
    title: attachment.title,
    url: attachment.url,
    hash: attachment.hash,
    addedAt: new Date().toISOString(),
    addedBy: req.body.actor ?? "system",
  };

  const notes = event.notes || {};
  const attachments = notes.attachments || [];
  
  const updated = {
    ...event,
    notes: {
      ...notes,
      attachments: [...attachments, newAttachment],
    },
    updatedAt: new Date().toISOString(),
  };

  await redis.set(EVENT_KEY(eventId), JSON.stringify(updated));

  await auditFromEvent({
    action: "RECEIPT_ATTACH",
    event: updated,
    actor: req.body.actor ?? "system",
    policyOk: true,
    meta: { attachmentAdded: newAttachment.id },
  });

  return res.json({ ok: true, event: updated, attachment: newAttachment });
}